(function() {
  ...
  document.addEventListener("DOMContentLoaded", function() {
    ...
    function run() {
      ...
      document.querySelector("#input .size").textContent = byteLength(
        JSON.stringify(JSON.parse(editors["input"].getValue()))
      );
      document.querySelector("#output .size").textContent = byteLength(
        JSON.stringify(JSON.parse(editors["output"].getValue()))
      );
      document.querySelector("#unpacked .size").textContent = byteLength(
        JSON.stringify(JSON.parse(editors["unpacked"].getValue()))
      );
    }
